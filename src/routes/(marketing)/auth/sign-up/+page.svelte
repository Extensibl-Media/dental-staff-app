			...
			{#if newUserType}
				<Card.Content class="grid gap-4">
					...
					<Form.Field {config} name="role">
						<Form.Item>
							<Form.Input type="hidden" value={newUserType} />
						</Form.Item>
					</Form.Field>
			...
			{:else}
				<CardContent>
					...
					<div class="my-4">
						<button
							type="button"
							on:click={() => (newUserType = 'CLIENT')}
							class="flex w-full justify-between py-4 border-b hover:bg-gray-200 px-2 rounded-sm"
						>
							Client
							<ChevronRight />
						</button>
						<button
							type="button"
							on:click={() => (newUserType = 'CANDIDATE')}
							class="flex w-full justify-between py-4 hover:bg-gray-200 px-2 rounded-sm"
						>
							Candidate
							<ChevronRight />
						</button>
