		...
		<Alert variant="destructive">
			...
			<AlertDescription>
				This timesheet has {data?.discrepancies.length} issue{data?.discrepancies.length > 1
					? 's'
					: ''} that need{data?.discrepancies.length === 1 ? 's' : ''} your review before approval. Check
				the Discrepancies tab for details.
			</AlertDescription>
		</Alert>
	{/if}
			...
			{#if data?.timesheet?.awaitingClientSignature}
				<Card>
					...
					<CardContent class="space-y-4">
						{#if data.timesheet.status === 'APPROVED' && data.invoice}
							<p class="text-sm text-gray-600">
								This timesheet has been approved and an invoice has been created for it.
							</p>
						{:else}
							<p class="text-sm text-gray-600">
